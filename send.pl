...
my $maxAtOnce = 0;	# max nbr of messages that are processed within one batch
my $pollPeriod = 1;
...
sub onReceive {
	...
	#	may also be in three hours...
		...
		#my @args = split (' ',$_); 
		#system($^X, "/usr/local/bin/zabbix_syslog_lkp_host.pl", @args);
		#system("/usr/local/bin/zabbix_syslog_lkp_host.pl", @_);
		local @ARGV = split (' ',$_ );
		do ("/usr/local/bin/zabbix_syslog_lkp_host.pl");
